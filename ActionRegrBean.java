...
@ManagedBean
@SessionScoped
public class ActionRegrBean {

	private int columnCount;
	private int rowsAffected;
	private String message;
	private FacesContext context1;
	...
	private List<StatisticsManagedBean> statisticList;
...
//	private dbAccessActionbean dbAccessActionbean;
	...
	private StatisticsManagedBean statisticsManagedBean;
	private RegrStatsBean regrStatsBean = new RegrStatsBean();
	...
	public ActionRegrBean()
	{
		columnSelected = new ArrayList<String>();
		columnsList = new ArrayList<String>();
		columns= new ArrayList<String>();	
		renderTabledata=false;
		statisticList= new ArrayList<StatisticsManagedBean>();
	...
	public void init() {
		...
		Map<String, Object> m = context1.getExternalContext().getSessionMap();
...
/*	public String processLogout()
	...
	public String getTables()
	{
		try {
			...
			tableList = new ArrayList<String>();
			String tablenames;
			resultSet = databaseAccess.getTables();
			if(resultSet != null)
			{
			...
			{
				message = databaseAccess.getMessage();
				renderMessage = true;
			}
			return "SUCCESS";
		}catch (Exception e) {
	...
	public boolean generateRegressionColumns()
	{
		try
		{
			String sqlQuery = "select * from " + databaseAccess.getSchema() +
					"." + tableSelected;		
			resultSet = databaseAccess.getColumnNames(sqlQuery);
			if(resultSet != null)
			{
				columnsList.clear();
...
public String calculateVariables()
	{
	    try
	    {
	    	...
	    	{
	    		System.out.println("asjkbdkasd"+databaseAccess.getSchema());
				String sqlQuery = "select " + list.get(k) + " from " + databaseAccess.getSchema() +
						"." + tableSelected;
				resultSet = databaseAccess.processSelect(sqlQuery);
				...
				for(int j=1;j<columnCount+1;j++)
				{
					...
					statisticList.add(new StatisticsManagedBean(columnName, minValue, maxValue,
							mean, variance, std, median, q1, q3, iqr, range));
					System.out.println(statisticList.get(0)+" "+ renderTabledata);
				//	statisticsManagedBean.setVariablesforGraph(q1, q3, median);
					}
				renderTabledata = true;
				System.out.println("vvsdfcsdf");
	    	}
	    	return "SUCCESS";
	    } catch(Exception e) {
	    	message = e.getMessage();
	...
	public String getColumnNames ()
	{
		try {
			...
			else
			{
				...
				String sqlQuery = "select * from " + databaseAccess.getSchema() +
						"." + tableSelected;
				ResultSet resultSet = databaseAccess.getColumnNames(sqlQuery);
				if(resultSet!=null)
				{
	...
	public boolean calculateRegressionVariables()
	{
		try {			
			...
			String sqlQuery = "select " + predictorValue + ", " + responseValue + 
					" from "+ databaseAccess.getSchema() + "." + tableSelected;
			resultSet = databaseAccess.processSelect(sqlQuery);
			if(resultSet!= null)
			{
				resultSetMetaData = (ResultSetMetaData) resultSet.getMetaData();
				...
				System.out.println(regressionEquation +","+ intercept +"\n"+  interceptStandardError +","+ 
							tStatistic + "\n"+ interceptPValue +","+  slope +"\n"+  slopeStandardError +","+ 
							tStatisticpredict +"\n"+  pValuePredictor + ","+ standardErrorModel +"\n"+ 
							rSquare + ","+ rSquareAdjusted + "\n"+ predictorDF + ","+ residualErrorDF +"\n"+ 
							totalDF +","+  regressionSumSquares + "\n"+ sumSquaredErrors +","+  totalSumSquares +"\n"+ 
							meanSquare +","+  meanSquareError + "\n"+ fValue + ","+ pValue);
				
				boolean regressionResultsStatus;
			
				regrStatsBean.setRegressionEquation(regressionEquation);
				/*regressionResultsStatus= regrStatsBean.setRegressionAnalysisVariables(
								 regressionEquation,intercept,
								 interceptStandardError,tStatistic,
								 interceptPValue,slope,
								 slopeStandardError,tStatisticpredict,
								 pValuePredictor,standardErrorModel,
								 rSquare,rSquareAdjusted,
								 predictorDF,residualErrorDF,
								 totalDF,regressionSumSquares,
								 ...
								 meanSquare,meanSquareError,
								 fValue,pValue);*/
				regressionResultsStatus= regrStatsBean.setRegressionAnalysisVariables(regressionEquation, intercept, interceptStandardError, tStatistic, interceptPValue, slope, slopeStandardError, tStatisticpredict, pValuePredictor, standardErrorModel, rSquare, rSquareAdjusted, predictorDF, residualErrorDF, totalDF, regressionSumSquares, sumSquaredErrors, totalSumSquares, meanSquare, meanSquareError, fValue, pValue);
				System.out.println(regressionResultsStatus);
				System.out.println(regrStatsBean.getRegressionEquation());
				System.out.println(regrStatsBean.getIntercept());
				System.out.println(regrStatsBean.getfValue());
				System.out.println(regrStatsBean.getpValue());
				...
				{
					message = regrStatsBean.getMessage();
					renderMessage = true;
					return false;
				}
			}
	...
	public List<StatisticsManagedBean> getStatisticList() {
		return statisticList;
	}

	public void setStatisticList(List<StatisticsManagedBean> statisticList) {
	...
	public StatisticsManagedBean getStatisticManagedBean() {
		return statisticsManagedBean;
	}

	public void setStatisticsManagedBean(StatisticsManagedBean statisticsManagedBean) {
		this.statisticsManagedBean = statisticsManagedBean;
	}
	...
	public RegrStatsBean getregrStatsBean() {
		return regrStatsBean;
	}

	public void setregrStatsBean(RegrStatsBean regrStatsBean) {
		this.regrStatsBean = regrStatsBean;
	}

	public StatisticsManagedBean getStatisticsManagedBean() {
		return statisticsManagedBean;
	}
